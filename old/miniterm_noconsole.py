...
class Miniterm(object):
    ...
    def dump_port_settings(self):
        ...
        print('--- RTS: %-8s  DTR: %-8s  BREAK: %-8s\n' % (
                (self.rts_state and 'active' or 'inactive'),
                (self.dtr_state and 'active' or 'inactive'),
                (self.break_state and 'active' or 'inactive')))
        try:
            print('--- CTS: %-8s  DSR: %-8s  RI: %-8s  CD: %-8s\n' % (
            ...
            pass
        print('--- software flow control: %s\n' % (self.serial.xonxoff and 'active' or 'inactive'))
        print('--- hardware flow control: %s\n' % (self.serial.rtscts and 'active' or 'inactive'))
        print('--- data escaping: %s  linefeed: %s\n' % (
            ...
            while self.alive and self._reader_alive:
                ...
                if self.repr_mode == 0:
                    ...
                    if self.convert_outgoing == CONVERT_CRLF and data in '\r\n':
                        ...
                        print(repr(data)[1:-1]) #sys.stdout.write(repr(data)[1:-1])
                    ...
                    for c in data:
                        print("%s " % c.encode('hex')) #sys.stdout.write("%s " % c.encode('hex'))
                #sys.stdout.flush()
                    ...
                    if c == MENUCHARACTER or c == EXITCHARCTER: # Menu character again/exit char -> send itself
                        ...
                        if port and port != self.serial.port:
                            ...
                            try:
                                ...
                                print('--- ERROR opening new port: %s ---\n' % (e,))
                                new_serial.close()
                            else:
                                self.serial.close()
                                self.serial = new_serial
                                print('--- Port changed to: %s ---\n' % (self.serial.port,))
                        ...
                        try:
                            ...
                            print('--- ERROR setting baudrate: %s ---\n' % (e,))
                        ...
                        print('--- unknown menu character %s --\n' % key_description(c))
                    menu_active = False
...
3: hex dump everything""",
    ...
    try:
        ...
        print("could not open port %r: %s\n" % (port, e))
        sys.exit(1)

    if not options.quiet:
        print('--- Miniterm on %s: %d,%s,%s,%s ---\n' % (
        ...
        print('--- Quit: %s  |  Menu: %s | Help: %s followed by %s ---\n' % (
